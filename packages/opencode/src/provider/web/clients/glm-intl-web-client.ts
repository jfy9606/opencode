...
const GLM_INTL_API_BASE = "https://chat.z.ai"
...
export class GLMIntlWebClient {
  ...
  async chatCompletions(params: {
    ...
    const res = await fetch(`${GLM_INTL_API_BASE}/chatglm/backend-api/assistant/stream`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) throw new Error(`GLM Intl API error (${res.status}): ${await res.text()}`)
    if (!res.body) throw new Error("GLM Intl returned empty response")
    return res.body
  }
